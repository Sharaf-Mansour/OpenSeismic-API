...
def Request_XML(startdate="2022-04-02", enddate="2024-05-02"):
    link = f"https://service.iris.edu/irisws/mars-event/1/query?starttime={startdate}T00:00:00&endtime={
        enddate}T00:00:00&magnitudetype=M2.4,MFB,MbS,MbP,MWspec,MW&eventtype=2.4Hz,HF,VF&locationquality=A,B,C&includeallorigins=false&includeallmagnitudes=false&includearrivals=true&version=14&orderby=time-asc&format=xml&nodata=204"
    r = requests.get(link)
...
# Define functions to construct the dataset
...
def get_picks(event):
    list_of_picks = ['P', 'S', 'p', 's', 'Pg', 'Sg', 'pP',
                     'P1', 'Pn', "PmP", "pwP", 'pwPm', 'S1', 'SmS', 'Sn']
...
def get_data(waveform):
    data = [i.data for i in waveform]
    min_len = min([len(x) for x in data])
    data = np.array([i[:min_len]for i in data])
    return data


def Construct_Dataset(start_date="2022-04-02", end_date="2022-05-02", data_folder="Waveforms", metadata_name="Metadata.csv", waveforms_name="Waveforms.hdf5"):

    Request_XML(start_date, end_date)
    print('Finished Writing XML file', '\n', '-'*60)
    base_path = Path(data_folder)
    metadata_path = base_path/metadata_name
    waveforms_path = base_path/waveforms_name
    print('Created Paths')
    with sbd.WaveformDataWriter(metadata_path, waveforms_path) as writer:
        ...
        print('Catalog read successfuly')
        for event in catalog:
            ...
            assert all(trace.stats.sampling_rate ==
                       sampling_rate for trace in waveforms)
            ...
            for pick in event.picks:
                ...
                trace_params[f"trace_{
                    pick.phase_hint}_arrival_sample"] = int(sample)
